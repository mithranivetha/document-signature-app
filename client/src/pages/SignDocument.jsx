...
import { useAuth, SESSION_ID } from '../context/AuthContext'
  ...
  const handlePDFClick = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5001/api/signatures', {
        documentId: id, x, y, page: 1, sigImage: pendingSig, sessionId: SESSION_ID
      }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F6F3', fontFamily: 'Georgia, serif' }}>
      <nav style={{ background: '#1C1C1E', padding: '0 48px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <a href="/dashboard" style={{ color: '#aaa', fontSize: '0.9rem', textDecoration: 'none' }}>
          Back to Dashboard
        </a>
      </nav>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <p style={{ color: '#aaa', fontSize: '0.9rem' }}>
            {placing
              ...
              : 'Place your signature, drag it to reposition, then click the green check to lock it in place'}
          </p>
        </div>
          ...
          {signatures.length > 0 && (
            <>
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#4CAF7D', fontSize: '0.9rem', fontWeight: '600' }}>
                <FontAwesomeIcon icon={faCheck} /> {signatures.length} signature{signatures.length > 1 ? 's' : ''} placed
              </span>
              <button
                onClick={handleFinalize}
                style={{ background: '#1C1C1E', color: '#F5A65B', border: '1.5px solid #F5A65B', borderRadius: '10px', padding: '10px 24px', fontSize: '0.9rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'Georgia, serif', display: 'flex', alignItems: 'center', gap: '8px' }}
              >
                <FontAwesomeIcon icon={faDownload} /> Finalize & Download
              </button>
            </>
          )}
        </div>
